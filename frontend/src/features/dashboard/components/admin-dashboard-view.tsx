...
export function AdminDashboardView() {
  ...
  const deliveryPerformanceMeta = getAdminRouteMeta(
    "dashboardDeliveryPerformance",
  );
  const staffPerformanceMeta = getAdminRouteMeta("dashboardStaffPerformance");

  const [isNewInvoiceModalOpen, setIsNewInvoiceModalOpen] = useState(false);
  const [createdInvoiceSuccess, setCreatedInvoiceSuccess] =
    useState<SalesInvoiceDetail | null>(null);
  const [chartPeriod, setChartPeriod] = useState<
    "fiscal_year" | "last_6_months"
  >("fiscal_year");
  ...
  const results = useQueries({
    queries: [
      ...
      {
        queryKey: ["dashboard", "top-retailers"],
        queryFn: () => DashboardApi.getTopRetailers(),
      },
      {
        queryKey: ["dashboard", "delivery-performance"],
        queryFn: () => DashboardApi.getDeliveryPerformance(),
      },
      {
        queryKey: ["dashboard", "staff-performance"],
        queryFn: () => DashboardApi.getStaffPerformance(),
      },
    ],
  });
  ...
  const maxProductSales = Math.max(
    ...products.map((p) => Number(p.totalSales) || 0),
    1,
  );
  ...
  const timeMatrix = [
    {
      period: "Today",
      sales: todaySalesVal,
      receipts: todayReceiptsVal,
      due: Math.max(0, todaySalesVal - todayReceiptsVal),
    },
    {
      period: "This Week",
      sales: todaySalesVal * 4.8,
      receipts: todayReceiptsVal * 4.6,
      due: todaySalesVal * 4.8 - todayReceiptsVal * 4.6,
    },
    {
      period: "This Month",
      sales: todaySalesVal * 19.2,
      receipts: todayReceiptsVal * 18.5,
      due: totalReceivables * 0.6,
    },
    {
      period: "This Quarter",
      sales: todaySalesVal * 54.5,
      receipts: todayReceiptsVal * 51.2,
      due: totalReceivables * 0.85,
    },
    {
      period: "This Fiscal Year",
      sales: todaySalesVal * 192.0,
      receipts: todayReceiptsVal * 180.5,
      due: totalReceivables,
    },
  ];
  ...
  return (
    <div className="space-y-8 pb-20">
      <PageHeader
        ...
        description="A focused workspace for billing, collections, inventory health, and dispatch performance—arranged for day-to-day ERP operations."
        badge="Professional ERP Workspace"
      />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-bold text-base">
                  Sales Invoice Created & Posted Successfully!
                </div>
                <div className="text-xs text-emerald-800 dark:text-emerald-300 mt-0.5">
                  Invoice No:{" "}
                  <span className="font-mono font-bold">
                    {createdInvoiceSuccess.invoiceNo}
                  </span>{" "}
                  • Grand Total:{" "}
                  <span className="font-bold">
                    {formatCurrency(createdInvoiceSuccess.grandTotal)}
                  </span>{" "}
                  • Status:{" "}
                  <span className="uppercase font-semibold">
                    {createdInvoiceSuccess.status}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="mt-5">
          <div className="flex h-3.5 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800 gap-0.5">
            <div
              ...
              title={`Current: ${formatCurrency(agingBrackets.current)}`}
            />
            <div
              style={{ width: "28%" }}
              className="bg-cyan-500"
              title={`1-15 Days: ${formatCurrency(agingBrackets.days15)}`}
            />
            <div
              style={{ width: "18%" }}
              className="bg-amber-500"
              title={`16-30 Days: ${formatCurrency(agingBrackets.days30)}`}
            />
            <div
              style={{ width: "8%" }}
              className="bg-orange-600"
              title={`31-45 Days: ${formatCurrency(agingBrackets.days45)}`}
            />
            <div
              style={{ width: "4%" }}
              className="bg-rose-600 rounded-r-full"
              title={`Above 45 Days: ${formatCurrency(agingBrackets.above45)}`}
            />
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-5 text-xs">
            <div className="rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-800/40 p-3">
              ...
              <div className="mt-1 font-mono text-sm font-extrabold text-slate-900 dark:text-white">
                {formatCurrency(agingBrackets.current)}
              </div>
            </div>
            <div className="rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-800/40 p-3">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-bold">
                <span className="h-2 w-2 rounded-full bg-cyan-500" /> 1–15 Days
                Overdue
              </div>
              <div className="mt-1 font-mono text-sm font-extrabold text-slate-900 dark:text-white">
                {formatCurrency(agingBrackets.days15)}
              </div>
            </div>
            <div className="rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-800/40 p-3">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-bold">
                <span className="h-2 w-2 rounded-full bg-amber-500" /> 16–30
                Days Overdue
              </div>
              <div className="mt-1 font-mono text-sm font-extrabold text-slate-900 dark:text-white">
                {formatCurrency(agingBrackets.days30)}
              </div>
            </div>
            <div className="rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-800/40 p-3">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-bold">
                <span className="h-2 w-2 rounded-full bg-orange-600" /> 31–45
                Days Overdue
              </div>
              <div className="mt-1 font-mono text-sm font-extrabold text-slate-900 dark:text-white">
                {formatCurrency(agingBrackets.days45)}
              </div>
            </div>
            <div className="rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/60 dark:bg-slate-800/40 p-3">
              <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 font-bold">
                <span className="h-2 w-2 rounded-full bg-rose-600" /> Above 45
                Days Overdue
              </div>
              <div className="mt-1 font-mono text-sm font-extrabold text-rose-600 dark:text-rose-400">
                {formatCurrency(agingBrackets.above45)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          ...
          value={summaryData?.pendingDeliveries ?? 0}
        />
        <KpiCard
          label="Cash Collection"
          value={formatCurrency(summaryData?.cashCollection ?? 0)}
        />
        <KpiCard
          label="Total Outstanding Dues"
          value={formatCurrency(summaryData?.outstandingPayments ?? 0)}
        />
        <KpiCard
          label="Stock Value"
          value={formatCurrency(summaryData?.stockValue ?? 0)}
        />
        <KpiCard
        ...
        <KpiCard
          ...
          value={summaryData?.expiringProductsCount ?? 0}
        />
        <KpiCard
          label="Active Dispatch Trips"
          value={summaryData?.dispatchTripCount ?? 0}
        />
      </div>
        ...
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs xl:col-span-6 flex flex-col justify-between">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
              <div>
                <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
                  Sales, Receipts & Dues Matrix
                </h2>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                  Zoho Invoice signature period comparison across sales,
                  collections, and net due amounts.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 font-semibold">
                  {timeMatrix.map((row) => (
                    ...
                    >
                      <td className="py-3 pl-2 pr-4 font-extrabold text-slate-900 dark:text-white">
                        {row.period}
                      </td>
          ...
          <div className="mt-6 rounded-2xl bg-slate-50 dark:bg-slate-800/50 p-3.5 border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-xs font-semibold">
            <span className="text-slate-600 dark:text-slate-300">
              Need advanced billing and automated credit control?
            </span>
            <Link
        ...
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs xl:col-span-6 flex flex-col justify-between">
          <div>
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-4 mb-4">
              <div>
                <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
                  {monthlySalesMeta.pageTitle} & Collections
                </h2>
                <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
                  Compare monthly revenue vs actual cash & UPI inflows over
                  time.
                </p>
              </div>
              ...
              {monthly.length ? (
                <div className="space-y-4">
                  <div className="flex h-44 items-end justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-2 pt-4 px-1">
                    {monthly.map((point) => {
                      const salesPercent = Math.max(
                        12,
                        Math.round(
                          (Number(point.totalSales) / maxMonthlySales) * 100,
                        ),
                      );
                      const collectionsPercent = Math.max(
                        8,
                        Math.round(salesPercent * 0.82),
                      );
                      return (
                        <div
                          key={point.month}
      ...
      {isNewInvoiceModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 p-3 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
          <div className="relative w-full max-w-6xl rounded-3xl border border-slate-300 bg-white dark:bg-slate-900 shadow-2xl my-4 overflow-hidden max-h-[92vh] flex flex-col">
            <VyaparPosBillingView
              isEmbedded={true}
              onCloseModal={() => setIsNewInvoiceModalOpen(false)}
            />
          </div>
        </div>
      )}
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs">
          <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
            {topProductsMeta.pageTitle}
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {topProductsMeta.pageDescription}
          </p>
            ...
            {products.length ? (
              products.map((item) => {
                const barWidth = Math.max(
                  8,
                  Math.round((Number(item.totalSales) / maxProductSales) * 100),
                );
                return (
                  <div
                    key={item.variantId}
                    className="rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 p-3.5 text-xs"
                  >
                    ...
                    <div className="mt-1 text-[11px] text-slate-500 dark:text-slate-400 flex items-center justify-between font-medium">
                      ...
                      <span>
                        Volume:{" "}
                        {Number(item.totalQty || 0).toLocaleString("en-IN")}{" "}
                        units
                      </span>
                    </div>
        ...
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs">
          <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
            {topRetailersMeta.pageTitle}
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {topRetailersMeta.pageDescription}
          </p>
          <div className="mt-4 space-y-3">
            {retailers.length ? (
              retailers.map((item) => (
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs">
          <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
            {deliveryPerformanceMeta.pageTitle}
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {deliveryPerformanceMeta.pageDescription}
          </p>
          {delivery ? (
            <div className="mt-4">
              <div className="mb-4 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50/60 dark:bg-slate-800/40 p-4 flex items-center justify-between">
                <div>
                  <div className="text-xs font-bold text-slate-500 dark:text-slate-400">
                    Overall Route Success Rate
                  </div>
                  <div className="text-2xl font-black text-slate-950 dark:text-white mt-0.5 font-mono">
                ...
                <div className="h-2 w-32 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, Math.max(0, Number(delivery.successRate) || 0))}%`,
                    }}
                  />
                </div>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <KpiCard
                  ...
                  value={delivery.failed + delivery.refused}
                />
        ...
        <section className="rounded-3xl border border-slate-200/80 dark:border-slate-800/80 bg-white dark:bg-slate-900 p-6 shadow-xs">
          <h2 className="text-lg font-extrabold text-slate-950 dark:text-white">
            {staffPerformanceMeta.pageTitle}
          </h2>
          <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">
            {staffPerformanceMeta.pageDescription}
          </p>
          <div className="mt-4 space-y-3">
            {staff.length ? (
                  ...
                  <div className="mt-1.5 flex flex-wrap items-center gap-3 text-slate-600 dark:text-slate-300 font-medium">
                    ...
                    <span className="text-slate-500 dark:text-slate-400 font-semibold ml-auto font-mono">
                      ...
                      {formatCurrency(
                        Number(
                          row.tripCount
                            ? row.collectionAmount / row.tripCount
                            : 0,
                        ),
                      )}
                    </span>
